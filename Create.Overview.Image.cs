...
// 
// Create an image to get an overview of all signal layers.
//-----------------------------------------------------------------------------------
// GUID newScript_635900140114543120
// EnableOn JobOpen
// Category Overview
// DLLImport C:\Program Files (x86)\Reference Assemblies\Microsoft\Framework\.NETFramework\v4.0\Microsoft.VisualBasic.dll;
...
namespace PCBIScript
{
    public class PScript : IPCBIScript
    {
        ...
        public void Execute(IPCBIWindow parent)
        {
            ...
            string Rows = Interaction.InputBox("How many rows do you wish?", "Setup", "2");
            int rows = 2;
                    ...
                    if (step.GetLayer(layername) is IODBLayer)
                    {
                        ...
                        if (AddDrillsToImage)
                        {
                            ...
                            if (AddCompToImageAsSeperateLayer)
                            {
                                ...
                                {
                                    drawList.Add(step.GetLayer(matrix.GetTopComponentLayer()));
                                }
                                if (matrix.GetBotSignalLayer() == layername)
                                {
                                    drawList.Add(step.GetLayer(matrix.GetBotComponentLayer()));
                                }
                            }
                            resultImage = step.GetBitmap(drawList, boundsStep, width, height, true, false, true, false);
